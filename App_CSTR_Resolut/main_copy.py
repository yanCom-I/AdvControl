...
class CSTRApp:
    ...
    # Controladores PIDs originais
        self.lc = PIDController(Kp=-60.0, Ki=-8.0, Kd=-1.0, dt=self.dt, output_limits=(0, 100))
        self.tc = PIDController(Kp=3.9, Ki=0.02358, Kd=0.0, dt=self.dt, output_limits=(0, 100))

        # --- NOVAS VARIÁVEIS MPC ---
        self.control_mode = tk.StringVar(value="PID") # Padrão inicia em PID
        
        self.mpc = CSTR_MPC(dt=self.dt, P=30, M=5)
    ...
    def setup_operation_tab(self):
        ...
        # --- NOVO: SELETOR DE MODO DE CONTROLE (PID vs MPC) ---
        mode_frame = ttk.LabelFrame(fp_frame, text="Estratégia de Controle")
        mode_frame.pack(side="left", padx=20, fill="y")
        
        ttk.Radiobutton(mode_frame, text="PID Clássico", variable=self.control_mode, value="PID", bootstyle="primary").pack(anchor="w", pady=2)
        ttk.Radiobutton(mode_frame, text="MPC Avançado", variable=self.control_mode, value="MPC", bootstyle="success").pack(anchor="w", pady=2)
    ...
    def update(self):
        ...
        # Pega a OP do PID (Padrão das Faceplates)
        op_l = self.fp_level.update(c_level)
        op_t = self.fp_temp.update(c_temp)

        # SOBRESCREVE A OP SE ESTIVER EM MODO MPC ---
        if self.control_mode.get() == "MPC":
            sp_level = self.fp_level.sp_var.get()
            sp_temp = self.fp_temp.sp_var.get()
            
           # Passa arrays de PVs [Nível, Temp] e SPs [Nível, Temp]
            # O controlador calcula a melhor combinação de ambas as OPs simultaneamente
            op_l, op_t = self.mpc.update([c_level, c_temp], [sp_level, sp_temp])
            
            # Atualiza os painéis visuais para mostrar as OPs do MPC
            if hasattr(self.fp_level, 'op_var'): self.fp_level.op_var.set(op_l)
            if hasattr(self.fp_temp, 'op_var'): self.fp_temp.op_var.set(op_t)
            # Força as Faceplates a mostrarem o valor da OP do MPC na GUI
            if hasattr(self.fp_level, 'op_var'): self.fp_level.op_var.set(op_l)
            if hasattr(self.fp_temp, 'op_var'): self.fp_temp.op_var.set(op_t)

        # Roda o passo de simulação do reator com as OPs selecionadas
